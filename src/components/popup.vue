    ...
    <Dialog as="div" class="relative z-50" @close="$emit('cancel')">
      ...
      <div class="fixed inset-0 flex items-center justify-center p-4">
        <DialogPanel class="max-w-md bg-white rounded-lg p-6 shadow-xl text-center">
          ...
          <p class="text-gray-600 mt-2">Tem certeza que deseja remover este item do carrinho?</p>
          <div class="mt-4 flex justify-center gap-4">
            <button class="bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700" @click="$emit('confirm')">
              Sim, desejo remover.
            </button>
            <button class="bg-gray-200 text-gray-700 px-4 py-2 rounded hover:bg-gray-300" @click="$emit('cancel')">
              Não.
            </button>
          </div>
        </DialogPanel>
