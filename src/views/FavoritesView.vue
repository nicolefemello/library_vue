...
<template>
  <section id="cart" class="p-5 lg:p-20">
    ...
    <img v-if="favoriteStore.favoriteProducts.length == 0" src="/favorites.png"
      alt="Não há nenhum produto nos favoritos!" class="object-cover m-auto">
    ...
    <div v-else>
      <table id="cart" class="grid mt-10 sm:m-5 w-auto">
        ...
        <tbody>
          <tr v-for="(favorite, index) in favoriteStore.favoriteProducts" :key="index"
            class="flex justify-between items-center border-b border-[#BDBDBD]">
            <td class="w-5/6 py-1">
              <div id="product-cart" class="flex gap-5 py-3">
                <img :src="favorite.volumeInfo.imageLinks?.smallThumbnail" :alt="favorite.volumeInfo.title"
                  class="h-[170px] rounded-sm object-cover" />
                <div class="grid">
                  <h4 class="text-lg lg:text-xl font-semibold text-[#382C2C]">
                    {{ favorite.volumeInfo.title }}
                  </h4>
                  <p class="text-base text-[#4F4C57] my-1">{{ favorite.volumeInfo.authors?.join(', ') }}</p>
            ...
            <td class="w-1/6 py-1">
              ...
              <popup :is-open="isOpen" @confirm="confirmRemove" @cancel="isOpen = false" local="dos favoritos" />
            </td>
          </tr>
        </tbody>
      </table>
